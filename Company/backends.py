...
class CustomCompanyUserAuth(object):
        ...
        try:
            ...
            if check_password(post_password, user.password):
                ...
                if login_type == '1':
                    ...
                    insert_data = Corporate_Approves_1_Login_Access_Token.objects.create(subgroup_authenticater_id=user.subgroup_id,access_token=gen_access_token,user_agent=user_info)
                elif login_type == '3':
                    insert_data = Corporate_Approves_2_Login_Access_Token.objects.create(group_authenticater_id=user.group_id, access_token=gen_access_token,user_agent=user_info)
                elif login_type == '4':
                    insert_data = Corporate_Spoc_Login_Access_Token.objects.create(spoc=user.spoc_id,access_token=gen_access_token, user_agent=user_info)
                elif login_type == 'agent':
    ...
    def get_user(user_id):
        try:
            ...
            request = get_request()
            print("get user function" + request.session['access_token'])
